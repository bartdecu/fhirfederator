...
public abstract class FederatedProvider implements IResourceProvider{
     ...
     * @param cr
     */
    protected FederatedProvider(FhirContext ctx, ClientRegistry cr, ResourceRegistry rr, Class<? extends IBaseResource> br) {
      this.ctx = ctx;
        this.br = br;
      this.cr = cr;
      this.rr = rr;
    ...
    protected Optional<IGenericClient> getClient(Class<?> action, IBaseResource resource){
        ...
        Optional<ResourceConfig> preferred = servers.stream().filter(x -> evaluate(action,resource,x)).findFirst();
